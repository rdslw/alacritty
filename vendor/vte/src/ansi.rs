...
pub trait Handler {
    ...
    /// Report the current working directory as a URI (OSC 7).
    fn set_current_directory(&mut self, _: String) {}
    ...
    T: Timeout,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // Report working directory. Reject possibly truncated parameter lists.
            b"7" if (2..crate::MAX_OSC_PARAMS).contains(&params.len()) => {
                if let Ok(uri) = String::from_utf8(params[1..].join(&b';')) {
                    self.handler.set_current_directory(uri);
                }
            },
